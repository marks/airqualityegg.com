  ...
  namespace :bikes do 
    ...
    namespace :data do
      ...
      task :upsert, :resource_id do |t, args|
        ...
        data = File.read("/Users/mark/Box\ Sync/Louisville\ Community\ Data/MiscFiles/From\ DurhamLabs/AQE_form_A3_2014-06-05.csv")
        CSV.parse(data, :col_sep => ",", :headers => true) do |row|
          ...
          # monitoring_data[:parameter] = case row["SENSOR"].upcase
          #   when "OZONE"
          #     "O3"
          ...
          #     "NO2"
          #   when "VOLATILE ORGANIC COMPOUNDS" || "VOLATILE ORGANIC"
          #     "VOC"
          #   when "HUMIDITY" || "HUMDITY"
          #     "RHUM"
          #   when "TEMPERATURE"
          #     "TEMP"
          #   else 
          #     row["SENSOR"].upcase
          # end 
          # monitoring_data[:unit] = case row["UNITS"].upcase
          #   when "%RH"
          #     "%"
          #   else 
          #     row["UNITS"].upcase
